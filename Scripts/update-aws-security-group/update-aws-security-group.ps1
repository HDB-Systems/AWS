...
If (!($AccessKeyID -and $SecretAccessKeyID -and $Region -and $SecurityGroup)) {
  Throw "You must supply a value for -AccessKeyID, -SecretAccessKeyID, -Region and -SecurityGroup parameters" 
}

# validating parms
If (!($Ipv4 -or $Ipv6)) {
  Throw "You must supply -Ipv4 and/or -Ipv6 parameters"
}
...
Catch {
  Log "Error setting AWS info, aborting..."
  Throw
}
...
If ($Ipv4) {
  $CidrIpv4 = Invoke-RestMethod https://api.ipify.org?format=json
  Log "External Ipv4 is $($CidrIpv4.ip)"
}

# get my external ipv6
If ($Ipv6) {
  $CidrIpv6 = Invoke-RestMethod https://api6.ipify.org?format=json
  Log "External Ipv6 is $($CidrIpv6.ip)"
}
  ...
  ForEach ($Service in $Services) {
    ...
    ForEach ($IpPermission in $IpPermissionsList) {
      ...
      ForEach ($Ipv4Range in $Ipv4RangesList) {
        ...
        $resultCmd = Revoke-EC2SecurityGroupIngress -GroupId $InfoObject.GroupId -IpPermissions $revokeIpPermission
...
Catch {
  Log "Error revoking outdated permissions, aborting..."
  Throw
}

# configuring permissions
Try {
  Log "Configuring permissions for this computer on security group"
...
Catch {
  Log "Error configuring permissions, aborting..."
  Throw
}

# grant permissions
Try {
  Log "Granting permissions for this computer on security group"
  ...
  ForEach ($IpPermission in $IpPermissionsList) {
    ...
    ForEach ($Ipv4Range in $Ipv4RangesList) {
      ...
      If (!$existIpPermission.Count -gt 0) {
        ...
        $resultCmd = Grant-EC2SecurityGroupIngress -GroupId $InfoObject.GroupId -IpPermissions $grantIpPermission
...
Catch {
  Log "Error granting permissions, aborting..."
  Throw
}
